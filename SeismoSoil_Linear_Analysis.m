function varargout = SeismoSoil_Linear_Analysis(varargin)
% SEISMOSOIL_LINEAR_ANALYSIS MATLAB code for SeismoSoil_Linear_Analysis.fig
%      SEISMOSOIL_LINEAR_ANALYSIS, by itself, creates a new SEISMOSOIL_LINEAR_ANALYSIS or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_LINEAR_ANALYSIS returns the handle to a new SEISMOSOIL_LINEAR_ANALYSIS or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_LINEAR_ANALYSIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_LINEAR_ANALYSIS.M with the given input arguments.
%
%      SEISMOSOIL_LINEAR_ANALYSIS('Property','Value',...) creates a new SEISMOSOIL_LINEAR_ANALYSIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Linear_Analysis_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Linear_Analysis_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeismoSoil_Linear_Analysis

% Last Modified by GUIDE v2.5 24-Aug-2014 23:39:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeismoSoil_Linear_Analysis_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Linear_Analysis_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Linear_Analysis is made visible.
function SeismoSoil_Linear_Analysis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeismoSoil_Linear_Analysis (see VARARGIN)

clc;

% Choose default command line output for SeismoSoil_Linear_Analysis
...
% UIWAIT makes SeismoSoil_Linear_Analysis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Linear_Analysis_OutputFcn(hObject, eventdata, handles) 
...
if isequal(profile_file_name_,0)
    disp('User did not select any files.');
else
    disp(['User selected ',fullfile(profile_dir_name_,profile_file_name_)]);
    start_dir0 = profile_dir_name_;
    
    handles.metricdata.profile_file_name = profile_file_name_;
    handles.metricdata.profile_dir_name = profile_dir_name_;
    
    % rename profile
    if findstr('profile_',profile_file_name_) % if the profile filename contains "profile_"
        sitecode_ = profile_file_name_(9:end-4);
    else
        sitecode_ = profile_file_name_;
    end
    ...
    % output folder name
    folder_name = sprintf('%s_Linear_Analysis_Results',sitecode_);
...
close SeismoSoil_Linear_Analysis;
SeismoSoil;
