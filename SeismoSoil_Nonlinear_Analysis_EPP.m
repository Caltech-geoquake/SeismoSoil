function varargout = SeismoSoil_Nonlinear_Analysis_EPP(varargin)
% SEISMOSOIL_NONLINEAR_ANALYSIS_EPP MATLAB code for SeismoSoil_Nonlinear_Analysis_EPP.fig
%      SEISMOSOIL_NONLINEAR_ANALYSIS_EPP, by itself, creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_EPP or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_NONLINEAR_ANALYSIS_EPP returns the handle to a new SEISMOSOIL_NONLINEAR_ANALYSIS_EPP or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_EPP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_NONLINEAR_ANALYSIS_EPP.M with the given input arguments.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_EPP('Property','Value',...) creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_EPP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Nonlinear_Analysis_EPP_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Nonlinear_Analysis_EPP_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SeismoSoil_Nonlinear_Analysis_EPP_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Nonlinear_Analysis_EPP_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Nonlinear_Analysis_EPP is made visible.
function SeismoSoil_Nonlinear_Analysis_EPP_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes SeismoSoil_Nonlinear_Analysis_EPP wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Nonlinear_Analysis_EPP_OutputFcn(hObject, eventdata, handles) 
...
handles.metricdata.already_imported_Tau_max = 0;
handles.metricdata.step2_complete = 0;
guidata(hObject,handles);
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "Tau_max" data file...';
[Tau_max_file_name,Tau_max_dir_name,filter_index] ...
    = uigetfile(filter_spec,dlg_title,start_dir0,'MultiSelect','off');

if isequal(Tau_max_file_name,0)
    disp('User did not select any files.');
else
    disp(['User selected ',fullfile(Tau_max_dir_name,Tau_max_file_name)]);
    start_dir0 = Tau_max_dir_name; % update "start dir"
    
    handles.metricdata.Tau_max_file_name = Tau_max_file_name;
    handles.metricdata.Tau_max_dir_name = Tau_max_dir_name;
...
% --- Executes on button press in pushbutton3b_calc_strength.
function pushbutton3b_calc_strength_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton3b_calc_strength (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if handles.metricdata.already_imported_Tau_max == 1
    choice = questdlg('You''ve already imported shear strength. Do this calculation will overwrite the imported data. Do it anyway?', ...
	'Caution', ...
	'Yes','No','No');
    switch choice
        case 'Yes'
    ...
    if decision == 1
        ...
        Su = calcSuFromVsProfile(profile);  % shear strength
        h = profile(1:end-1,1);
        plotVsProfileFromArrays(h,Su/1000,sum(h));
        xlabel('Shear strength [kPa]');
        handles.metricdata.Tau_max = Su';
        guidata(hObject,handles);
    end
else
        profile = handles.metricdata.profile;
...
warning('off','all'); % supresses all warning messages
...
%     mkdir(output_dir);
    ...
    tic;
    [ok_to_proceed,h_running] = runNLEppFromGUI(profile,Tau_max,nr_motion,motion,motion_name,output_dir,...
...
close SeismoSoil_Nonlinear_Analysis_EPP;
SeismoSoil;
...
% --- Executes on button press in pushbutton20_post_processing.
function pushbutton20_post_processing_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton20_post_processing (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
