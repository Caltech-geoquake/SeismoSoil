function varargout = SeismoSoil_Nonlinear_Analysis_HH(varargin)
% SEISMOSOIL_NONLINEAR_ANALYSIS_HH MATLAB code for SeismoSoil_Nonlinear_Analysis_HH.fig
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HH, by itself, creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_HH or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_NONLINEAR_ANALYSIS_HH returns the handle to a new SEISMOSOIL_NONLINEAR_ANALYSIS_HH or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_NONLINEAR_ANALYSIS_HH.M with the given input arguments.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HH('Property','Value',...) creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_HH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Nonlinear_Analysis_HH_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Nonlinear_Analysis_HH_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SeismoSoil_Nonlinear_Analysis_HH_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Nonlinear_Analysis_HH_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Nonlinear_Analysis_HH is made visible.
function SeismoSoil_Nonlinear_Analysis_HH_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes SeismoSoil_Nonlinear_Analysis_HH wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Nonlinear_Analysis_HH_OutputFcn(hObject, eventdata, handles) 
...
if isequal(profile_file_name,0)
    ...
    folder_name = sprintf('%s_Nonlinear_Analysis_(HH)_Results',sitecode_);
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "HH_G" data file...';
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "HH_x" data file...';
...
close SeismoSoil_Nonlinear_Analysis_HH;
SeismoSoil;
