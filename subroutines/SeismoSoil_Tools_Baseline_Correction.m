function varargout = SeismoSoil_Tools_Baseline_Correction(varargin)
% SEISMOSOIL_TOOLS_BASELINE_CORRECTION M-file for SeismoSoil_Tools_Baseline_Correction.fig
%      SEISMOSOIL_TOOLS_BASELINE_CORRECTION, by itself, creates a new SEISMOSOIL_TOOLS_BASELINE_CORRECTION or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_TOOLS_BASELINE_CORRECTION returns the handle to a new SEISMOSOIL_TOOLS_BASELINE_CORRECTION or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_TOOLS_BASELINE_CORRECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_TOOLS_BASELINE_CORRECTION.M with the given input arguments.
%
%      SEISMOSOIL_TOOLS_BASELINE_CORRECTION('Property','Value',...) creates a new SEISMOSOIL_TOOLS_BASELINE_CORRECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Tools_Baseline_Correction_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Tools_Baseline_Correction_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeismoSoil_Tools_Baseline_Correction

% Last Modified by GUIDE v2.5 24-Aug-2014 23:42:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeismoSoil_Tools_Baseline_Correction_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Tools_Baseline_Correction_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Tools_Baseline_Correction is made visible.
function SeismoSoil_Tools_Baseline_Correction_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeismoSoil_Tools_Baseline_Correction (see VARARGIN)

% Choose default command line output for SeismoSoil_Tools_Baseline_Correction
...
% UIWAIT makes SeismoSoil_Tools_Baseline_Correction wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Tools_Baseline_Correction_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in pushbutton9_note_on_units.
function pushbutton9_note_on_units_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton9_note_on_units (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
handles.metricdata.cutoff_frequency = cutoff_frequency;
guidata(hObject,handles);
...
cutoff_frequency = str2double(get(hObject,'String'));
handles.metricdata.cutoff_frequency = cutoff_frequency;
guidata(hObject,handles);
...
if handles.metricdata.step4_complete == 0
    ...
    cutoff_frequency = handles.metricdata.cutoff_frequency;
    
    for i = 1 : 1 : nr_motion
        current_motion_filename = handles.metricdata.motion_file_name{i};
        current_motion = motion{i};
        
        baselineCorrHighPass(current_motion,cutoff_frequency,1,0,0);
        title(sprintf('"%s"\nBaseline corrected',current_motion_filename),'interpreter','none','fontsize',14);
    end
...
if handles.metricdata.step4_complete == 0
    ...
    for i = 1 : 1 : nr_motion
        ...
        baseline_result = baselineCorrHighPass(current_motion,cutoff_frequency,1,0,0);
        title(sprintf('"%s"\nBaseline corrected',current_motion_filename),'interpreter','none','fontsize',14);
...
% --- Executes on button press in pushbutton10_close_all.
function pushbutton10_close_all_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton10_close_all (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
