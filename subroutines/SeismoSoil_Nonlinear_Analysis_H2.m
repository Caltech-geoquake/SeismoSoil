function varargout = SeismoSoil_Nonlinear_Analysis_H2(varargin)
% SEISMOSOIL_NONLINEAR_ANALYSIS_H2 MATLAB code for SeismoSoil_Nonlinear_Analysis_H2.fig
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H2, by itself, creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_H2 or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_NONLINEAR_ANALYSIS_H2 returns the handle to a new SEISMOSOIL_NONLINEAR_ANALYSIS_H2 or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_NONLINEAR_ANALYSIS_H2.M with the given input arguments.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H2('Property','Value',...) creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_H2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Nonlinear_Analysis_H2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Nonlinear_Analysis_H2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeismoSoil_Nonlinear_Analysis_H2

% Last Modified by GUIDE v2.5 20-May-2015 12:40:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeismoSoil_Nonlinear_Analysis_H2_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Nonlinear_Analysis_H2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Nonlinear_Analysis_H2 is made visible.
function SeismoSoil_Nonlinear_Analysis_H2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeismoSoil_Nonlinear_Analysis_H2 (see VARARGIN)

clc;

% Choose default command line output for SeismoSoil_Nonlinear_Analysis_H2
...
% UIWAIT makes SeismoSoil_Nonlinear_Analysis_H2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Nonlinear_Analysis_H2_OutputFcn(hObject, eventdata, handles) 
...
[H2n_file_name,H2n_dir_name,filter_index] ...
    = uigetfile(filter_spec,dlg_title,start_dir0,'MultiSelect','off');

if isequal(H2n_file_name,0)
    disp('User did not select any files.');
else
    disp(['User selected ',fullfile(H2n_dir_name,H2n_file_name)]);
    start_dir0 = H2n_dir_name; % update "start dir"
    
    handles.metricdata.H2n_file_name = H2n_file_name;
    handles.metricdata.H2n_dir_name = H2n_dir_name;
    
    H2n_ = importdata(fullfile(H2n_dir_name,H2n_file_name));
    handles.metricdata.H2n = H2n_;
    
    checkInputs(H2n_,'h2n');
...
if step1*step2*step3*step4 == 0 % if a certain step is not completed
    ...
    checkInputs({vs_profile,curve,H2n},'all_h2');
    ...
    tic;
    [ok_to_proceed,h_running] = runNLH2FromGUI(vs_profile, curve, H2n,...
...
close SeismoSoil_Nonlinear_Analysis_H2;
SeismoSoil;
