...
if ok_to_proceed == 1
    ...
    parfor (ii = 1 : nr_motion, nr_cores)
        ...
        if ispc()
            if use_fortran == 1
                if isdeployed()
                    copyfile('NLH4.exe',dir_H4);
                else
                    copyfile(fullfile(fortran_dir,'NLH4.exe'),dir_H4);
                end
            end
        elseif ismac()
            if use_fortran == 1
                if isdeployed()
                    copyfile('/Applications/SeismoSoil.app/Contents/MacOS/NLH4.mac',dir_H4);
                else
                    copyfile(fullfile(fortran_dir,'NLH4.mac'),dir_H4);
                end
            end
        elseif isunix()
            copyfile(fullfile(fortran_dir,'NLH4.unix'),dir_H4);
        end
        ...
        [layer_depth,node_depth,out_a,out_v,out_d,out_gamma,out_tau] = ...
            resp_NLH4(f_max,ppw,n_dt,boundary,N_spr,accel_in,...
...
dlmwrite('H4_x.dat', para_x);

if ispc()
    if use_fortran == 1
        system('NLH4.exe');
    else
        nlH4; % run function "nlH4" -- no input args, no output args
    end
elseif ismac()
    if use_fortran == 1
        system('./NLH4.mac');
