function varargout = SeismoSoil_Nonlinear_Analysis_H4(varargin)
% SEISMOSOIL_NONLINEAR_ANALYSIS_H4 MATLAB code for SeismoSoil_Nonlinear_Analysis_H4.fig
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H4, by itself, creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_H4 or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_NONLINEAR_ANALYSIS_H4 returns the handle to a new SEISMOSOIL_NONLINEAR_ANALYSIS_H4 or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_NONLINEAR_ANALYSIS_H4.M with the given input arguments.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_H4('Property','Value',...) creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_H4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Nonlinear_Analysis_H4_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Nonlinear_Analysis_H4_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SeismoSoil_Nonlinear_Analysis_H4_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Nonlinear_Analysis_H4_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Nonlinear_Analysis_H4 is made visible.
function SeismoSoil_Nonlinear_Analysis_H4_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes SeismoSoil_Nonlinear_Analysis_H4 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Nonlinear_Analysis_H4_OutputFcn(hObject, eventdata, handles) 
...
if isequal(profile_file_name,0)
    ...
    folder_name = sprintf('%s_Nonlinear_Analysis_(H4)_Results',sitecode_);
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "H4_G" data file...';
...
if handles.metricdata.already_import_H4G == 1
    choice = questdlg('You''ve already imported H4_G data. Do this calculation will overwrite the imported data. Do it anyway?', ...
	'Caution', ...
	'Yes','No','No');
    switch choice
        case 'Yes'
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "H4_x" data file...';
...
if handles.metricdata.already_import_H4x == 1
    choice = questdlg('You''ve already imported H4_x data. Do this calculation will overwrite the imported data. Do it anyway?', ...
	'Caution', ...
	'Yes','No','No');
    switch choice
        case 'Yes'
...
% --- Executes during object creation, after setting all properties.
function checkbox4_use_parallel_CreateFcn(hObject, eventdata, handles)
% hObject    handle to checkbox4_use_parallel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

handles.metricdata.use_parallel = 0;
guidata(hObject,handles);


% --- Executes on button press in checkbox4_use_parallel.
function checkbox4_use_parallel_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox4_use_parallel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox4_use_parallel

use_parallel = get(hObject,'Value');
handles.metricdata.use_parallel = use_parallel;
guidata(hObject,handles);
...
use_parallel = handles.metricdata.use_parallel;
if use_parallel == 1
...
if step1*step2*step3*step4 == 0 % if a certain step is not completed
    ...
    checkInputs({vs_profile,curve,H4G,H4x},'all_h4');
    ...
    tic;
    [ok_to_proceed,h_running] = runNLH4FromGUI(vs_profile, curve, H4G, H4x,...
...
close SeismoSoil_Nonlinear_Analysis_H4;
SeismoSoil;
