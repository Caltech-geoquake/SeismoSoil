function [ok_to_proceed,h_running] = runNLHyFromGUI(profile,curve,H4G,H4x,...
    nr_motion,motion,motion_name,output_dir,...
    factor_rho,factor_xi,unit_factor_accel,bedrock_type,motion_type,...
    fig_visible,use_fortran)
% [ok_to_proceed,h_running] = runNLH4FromGUI(profile,curve,H4G,H4x,
%                             nr_motion,motion,motion_name,output_dir,
%                             factor_rho,factor_xi,unit_factor_accel,
%                             bedrock_type,motion_type,fig_visible)
% * * NOTES * *
%   factor_rho = 1 -- kg/m3, 1/1000 -- g/cm3
%   factor_xi = 1 -- unity, 100 -- %
%   unit_factor_accel = 1: m/s/s, 100: gal, 1/9.81, g
%   bedrock_type = 'elastic' or 'rigid'
%   motion_type = 'incident', 'borehole', or 'outcrop'
%
% version 11/18/2013
...
%                 copyfile(fullfile(fortran_exec_dir,'NLH4'),dir_H4);
                copyfile('/Applications/SeismoSoil.app/Contents/MacOS/NLHy',dir_H4);
            end
        end
        
        %% Reconstruct "tabk.dat"
        dlmwrite(fullfile(dir_H4,'tabk.dat'),tabk,'delimiter','\t');
...
%         end
        ...
        [layer_depth,node_depth,out_a,out_v,out_d,out_gamma,out_tau] = ...
            resp_NLHy(f_max,ppw,n_dt,boundary,N_spr,accel_in,...
            new_profile,curve,H4G,H4x,dir_H4,use_fortran);
        %     accel_in(:,2) = accel_in(:,2)/100;
...
function [ layer_depth, node_depth, out_a, out_v, out_d, out_gamma, out_tau ] = ...
    resp_NLHy(f_max, ppw, n_dt, boundary, N_spr, acc, profile, curve, para_G, para_x, dir_H4, use_fortran )
...
dlmwrite('Hy_G.dat', para_G);
dlmwrite('Hy_x.dat', para_x);

if strcmpi(computer,'pcwin64')
    if use_fortran == 1
        system('NLHy');
    else
        nlHy; % run function "nlHy" -- no input args, no output args
    end
elseif strcmpi(computer,'maci64')
    if use_fortran == 1
        system('./NLHy');
