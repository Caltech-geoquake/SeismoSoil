function varargout = SeismoSoil_Linear_Analysis_Time_Domain(varargin)
% SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN MATLAB code for SeismoSoil_Linear_Analysis_Time_Domain.fig
%      SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN, by itself, creates a new SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN returns the handle to a new SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN.M with the given input arguments.
%
%      SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN('Property','Value',...) creates a new SEISMOSOIL_LINEAR_ANALYSIS_TIME_DOMAIN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Linear_Analysis_Time_Domain_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Linear_Analysis_Time_Domain_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SeismoSoil_Linear_Analysis_Time_Domain_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Linear_Analysis_Time_Domain_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Linear_Analysis_Time_Domain is made visible.
function SeismoSoil_Linear_Analysis_Time_Domain_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes SeismoSoil_Linear_Analysis_Time_Domain wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Linear_Analysis_Time_Domain_OutputFcn(hObject, eventdata, handles) 
...
if isequal(profile_file_name,0)
    ...
    % output folder name
    folder_name = sprintf('%s_Linear_Analysis_(Time_Domain)_Results',sitecode_);
    ...
    handles.metricdata.f0_ro = f0_ro;
    handles.metricdata.AF_in = AF_in;
    handles.metricdata.TF_in = TF_in;
    handles.metricdata.AF_bh = AF_bh;
    handles.metricdata.TF_bh = TF_bh;
    handles.metricdata.f0_bh = f0_bh;
...
function pushbutton1d_view_lin_TF_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton1d_view_lin_TF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

bedrock_type = handles.metricdata.bedrock_type;
freq_array = handles.metricdata.freq_array;
switch bedrock_type
    case 'Elastic'
        AF = handles.metricdata.AF_ro;
        f0 = handles.metricdata.f0_ro;
    case 'Rigid'
        AF = handles.metricdata.AF_bh;
        f0 = handles.metricdata.f0_bh;
end
figure;
semilogx(freq_array,AF,'k','linewidth',1.5); hold on;
plot(f0,AF(find(freq_array==f0)),'ro','linewidth',1.75);
annotation('textbox',[.15 .75 .1 .1],'String',sprintf('f_0 = %.2f Hz',f0),'fontsize',13,'backgroundcolor','w');
xlabel('Frequency (Hz)','fontsize',12);
xlim([min(freq_array) max(freq_array)]);
ylim([0 max(AF)*1.15]);
ylabel('Amplification Factor','fontsize',12);
set(gca,'fontsize',12);
grid on;
title_text = sprintf('%s\nLinear Amplification Factor',handles.metricdata.sitecode);
title(title_text,'interpreter','none','fontsize',14);



% --- Executes on button press in pushbutton1e_save_lin_TF.
function pushbutton1e_save_lin_TF_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton1e_save_lin_TF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global start_dir0;
output_dir = uigetdir(start_dir0,'Select directory for exporting results...');
...
switch bedrock_type
    case 'Elastic'
        AF = handles.metricdata.AF_ro;
        TF = handles.metricdata.TF_ro;
    case 'Rigid'
        AF = handles.metricdata.AF_bh;
        TF = handles.metricdata.TF_ro;
end
sitecode = handles.metricdata.sitecode;
filename_real = sprintf('%s_linear_amplification_factor_real.dat',sitecode);
filename_comp = sprintf('%s_linear_amplification_factor_complex.dat',sitecode);
dlmwrite(fullfile(output_dir,filename_real),[freq_array,AF],'delimiter','\t');
dlmwrite(fullfile(output_dir,filename_comp),[freq_array,TF],'delimiter','\t');

command_Windows = sprintf('explorer.exe %s',output_dir);
command_Mac = sprintf('open %s',output_dir);
...
if get(hObject,'Value')
    ...
    input_accel_unit = 'gal';
    handles.metricdata.accel_unit_division_factor = accel_unit_division_factor;
    handles.metricdata.input_accel_unit = input_accel_unit;
    guidata(hObject,handles);
end
...
if handles.metricdata.step2_complete == 0
...
% --- Executes during object creation, after setting all properties.
function checkbox3_use_parallel_CreateFcn(hObject, eventdata, handles)
% hObject    handle to checkbox3_use_parallel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in checkbox3_use_parallel.
function checkbox3_use_parallel_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox3_use_parallel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox3_use_parallel
...
str1 = ' ';
if step1*step2 == 0 % if a certain step is not completed
    for i = 1 : 1 : 2
        if step(i) == 0
            str1 = [str1,num2str(i),','];
        end
...
%     mkdir(output_dir);
    ...
    tic;
    [ok_to_proceed,h_running] = runLinTimeDomainFromGUI(profile,nr_motion,motion,motion_name,output_dir,...
...
close SeismoSoil_Linear_Analysis_Time_Domain;
SeismoSoil;

% --- Executes on button press in pushbutton21_close_all.
function pushbutton21_close_all_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton21_close_all (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
