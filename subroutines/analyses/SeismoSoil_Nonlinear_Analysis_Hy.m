function varargout = SeismoSoil_Nonlinear_Analysis_Hy(varargin)
% SEISMOSOIL_NONLINEAR_ANALYSIS_HY MATLAB code for SeismoSoil_Nonlinear_Analysis_Hy.fig
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HY, by itself, creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_HY or raises the existing
%      singleton*.
%
%      H = SEISMOSOIL_NONLINEAR_ANALYSIS_HY returns the handle to a new SEISMOSOIL_NONLINEAR_ANALYSIS_HY or the handle to
%      the existing singleton*.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEISMOSOIL_NONLINEAR_ANALYSIS_HY.M with the given input arguments.
%
%      SEISMOSOIL_NONLINEAR_ANALYSIS_HY('Property','Value',...) creates a new SEISMOSOIL_NONLINEAR_ANALYSIS_HY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeismoSoil_Nonlinear_Analysis_Hy_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeismoSoil_Nonlinear_Analysis_Hy_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeismoSoil_Nonlinear_Analysis_Hy

% Last Modified by GUIDE v2.5 09-Feb-2015 00:46:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeismoSoil_Nonlinear_Analysis_Hy_OpeningFcn, ...
                   'gui_OutputFcn',  @SeismoSoil_Nonlinear_Analysis_Hy_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeismoSoil_Nonlinear_Analysis_Hy is made visible.
function SeismoSoil_Nonlinear_Analysis_Hy_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeismoSoil_Nonlinear_Analysis_Hy (see VARARGIN)

clc;

% Choose default command line output for SeismoSoil_Nonlinear_Analysis_Hy
...
% UIWAIT makes SeismoSoil_Nonlinear_Analysis_Hy wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeismoSoil_Nonlinear_Analysis_Hy_OutputFcn(hObject, eventdata, handles) 
...
if isequal(profile_file_name,0)
    ...
    start_dir0 = profile_dir_name; % update start dir
    ...
    folder_name = sprintf('%s_Nonlinear_Analysis_(Hy)_Results',sitecode_);
...
% hObject    handle to uipanel13_mass_density (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes when selected object is changed in uipanel13_mass_density.
function uipanel13_mass_density_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in uipanel13_mass_density 
...
% --- Executes during object creation, after setting all properties.
function pushbutton3Aa_select_H4G_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pushbutton3Aa_select_H4G (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

handles.metricdata.already_import_H4G = 0;
handles.metricdata.step3A_complete = 0;
handles.metricdata.step3_complete = 0;
guidata(hObject,handles);


% --- Executes on button press in pushbutton3Aa_select_H4G.
function pushbutton3Aa_select_H4G_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton3Aa_select_H4G (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "Hy_G" data file...';
[H4G_file_name,H4G_dir_name,filter_index] ...
    = uigetfile(filter_spec,dlg_title,start_dir0,'MultiSelect','off');

if isequal(H4G_file_name,0)
    disp('User did not select any files.');
else
    disp(['User selected ',fullfile(H4G_dir_name,H4G_file_name)]);
    start_dir0 = H4G_dir_name; % update "start dir"
    
    handles.metricdata.H4G_file_name = H4G_file_name;
    handles.metricdata.H4G_dir_name = H4G_dir_name;
...
if handles.metricdata.already_import_H4G == 1
    choice = questdlg('You''ve already imported Hy_G data. Do this calculation will overwrite the imported data. Do it anyway?', ...
	'Caution', ...
	'Yes','No','No');
    switch choice
        case 'Yes'
...
function pushbutton3Ba_select_H4x_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton3Ba_select_H4x (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
filter_spec = {'*.dat;*.txt','Text files (*.dat,*.txt)';'*.*', 'All Files (*.*)'};
dlg_title = 'Select "Hy_x" data file...';
[H4x_file_name,H4x_dir_name,filter_index] ...
    = uigetfile(filter_spec,dlg_title,start_dir0,'MultiSelect','off');

if isequal(H4x_file_name,0)
    disp('User did not select any files.');
else
    disp(['User selected ',fullfile(H4x_dir_name,H4x_file_name)]);
    start_dir0 = H4x_dir_name; % update "start dir"

    handles.metricdata.H4x_file_name = H4x_file_name;
    handles.metricdata.H4x_dir_name = H4x_dir_name;
...
if handles.metricdata.already_import_H4x == 1
    choice = questdlg('You''ve already imported Hy_x data. Do this calculation will overwrite the imported data. Do it anyway?', ...
	'Caution', ...
	'Yes','No','No');
    switch choice
        case 'Yes'
...
% --- Executes on button press in checkbox3_use_fortran_for_calc.
function checkbox3_use_fortran_for_calc_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox3_use_fortran_for_calc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox3_use_fortran_for_calc

use_fortran_for_calc = get(hObject,'Value');
handles.metricdata.use_fortran_for_calc = use_fortran_for_calc;
guidata(hObject,handles);



% --- Executes during object creation, after setting all properties.
function checkbox3_use_fortran_for_calc_CreateFcn(hObject, eventdata, handles)
% hObject    handle to checkbox3_use_fortran_for_calc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

handles.metricdata.use_fortran_for_calc = 1; % default: checked
guidata(hObject,handles);
...
step1 = handles.metricdata.step1_complete;  % soil profile
step2 = handles.metricdata.step2_complete;  % G/Gmax and damping curves
step3 = handles.metricdata.step3_complete;  % Parameters
step4 = handles.metricdata.step4_complete;  % Ground motion
step = [step1,step2,step3,step4];

str1 = ' ';
if step1*step2*step3*step4 == 0 % if a certain step is not completed
    for i = 1 : 1 : 4
        if step(i) == 0
            str1 = [str1,num2str(i),','];
        end
...
%     mkdir(output_dir);
    ...
    tic;
    [ok_to_proceed,h_running] = runNLHyFromGUI(profile,curve,H4G,H4x,nr_motion,motion,motion_name,output_dir,...
        factor_rho,factor_xi,factor_accel,bedrock_type,motion_type,fig_visible_option,use_fortran_for_calc);
    elapsed_time = toc;
...
close SeismoSoil_Nonlinear_Analysis_Hy;
SeismoSoil;
...
% --- Executes on button press in pushbutton22_post_processing.
function pushbutton22_post_processing_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton22_post_processing (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
