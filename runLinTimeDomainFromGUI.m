function [ok_to_proceed,h_running] = runLinTimeDomainFromGUI(profile,nr_motion,motion,motion_name,output_dir,...
...
if ok_to_proceed == 1
    ...
    %% Make up H2n and curve
    max_material_nr = max(material_nr);
    % curve = 2;
    H2n = [0.1*ones(1,max_material_nr);zeros(1,max_material_nr);...
        0.3*ones(1,max_material_nr);0.4*ones(1,max_material_nr)]; % I made up the 0.1, 0.3 and 0.4 values...
    para = H2n;
    
    curve = zeros(10,max_material_nr*4);
    for i = 1 : 1 : max_material_nr*4
        if (mod(i,4) == 1) || (mod(i,4) == 3)
            curve(:,i) = [0.0001 0.0003 0.001 0.003 0.01 0.03 0.1 0.3 1 3]';
        elseif mod(i,4) == 2
            curve(:,i) = ones(10,1);
        else
            curve(:,i) = 0.07*ones(10,1); % I made up this 0.07 value...
        end
    end
...
% %         n_dt = ceil(dt/dt_cr)*2; % times 2 for extra safety
        ...
        %% Doing time-domain linear analysis
% %         if strcmpi(computer,'pcwin64')
            current_dir = pwd;
% %         elseif strcmpi(computer,'maci64')
...
% %         end
        ...
        %         resp_H2rev(f_max,ppw,n_dt,boundary,N_spr,accel_in,...
        %         new_profile,curve,para,dir_h2rev);
        [layer_depth,node_depth,out_a,out_v,out_d,out_gamma,out_tau] = ...
            resp_TDLinear(f_max,ppw,n_dt,boundary,N_spr,accel_in,...
            new_profile,curve,para,dir_h2rev);
        
        %% Process output files -- 1
        out_a = importdata('out_a.dat'); %out_a = fixBugOfE(out_a);
        ...
        filename_TF_raw = sprintf('%s_linear_TF_raw%s',motion_name_without_ext,ext);
        %     filename_TF_smoothed = sprintf('%s_linear_TF_smoothed%s',motion_name_without_ext,ext);
        ...
        subplot(211);
        semilogx(freq_array,tf0,'k','linewidth',1.75); hold on;
        ...
        ylim([0 max(tf0(find(freq_array >= 0.1)))*1.35]);
        tf_title_text = sprintf('Linear Ampl. Factor (Time Domain)\n%s',motion_name_without_ext);
        title(tf_title_text,'interpreter','none','fontsize',fz_title);
        
        subplot(212);
        plot(freq_array,tf0,'k','linewidth',1.75); hold on;
        ...
        tf_fig_filename = sprintf('%s_linear_TF.png',motion_name_without_ext);
...
%    resp_H2(f_max, ppw, n_dt, boundary, N_spr, acc, profile, curve, para, dir_H2 )
%
...
function [ layer_depth, node_depth, out_a, out_v, out_d, out_gamma, out_tau ] = ...
    resp_TDLinear(f_max, ppw, n_dt, boundary, N_spr, acc, profile, curve, para, dir_H2 )
